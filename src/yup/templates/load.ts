...
@Component({
    selector: 'yup-tip',
    template: `
        <div class="yup-mask-transparent" [@mask]="disp" *ngIf="data.mask"></div>
        <div class="yup-tip" [@mask]="disp" *ngIf="disp">
            <i [ngClass]="icon(data.type)" class="yup-icon_tip"></i>
            <p class="yup-tip__content">{{data?.msg || ''}}</p>
        </div>
    `,
    animations: [mask],
    styles: [`
        ...
        .yup-icon-success-no-circle{
            font: normal normal normal 23px/1 "weui";
                font-size: 23px;
                color: #09BB07;
        }
...
export class LoadComponent {
    ...
    public dialogRef: YupRef<LoadComponent>;
    public disp: string;
    constructor(
